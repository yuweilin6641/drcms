    ...
    $.contextMenu({
        ...
        callback: function(key, options) { 
			if(key=='showDetail'){
				...
				location.href='/drcms/rma/detail?id='+dIndex;
			}
        },
        items: {
...
<h1 class="contentTitle">RMA資料維護</h1>
<div align="center">
	...
	function showTable(){
		...
		$("#jqGridTable").jqGrid({
		    ...
		    onPaging: function(pgButton){ 
				$("#jqGridTable").setGridParam({url:'rma/list' + processUrl()});
			},
		});
		$("#jqGridTable").jqGrid('navGrid','#jqGridPager',{edit:false,add:false,del:false,search:false});
		$("#jqGridTable").setGridParam({url:'rma/list' + processUrl()}).trigger("reloadGrid");
	}
	
	function formatIssueId (cellvalue, options, rowObject){
		return "<a href='/drcms/rma/detail?id="+options.rowId+"'>"+options.rowId+"</a>";
	}
